...
-export([request_target/3, send_result/3, send_ready/3]).
...
-spec send_ready(Server::pid(), Ref::term(), Ready::boolean()) -> ok.
send_ready(Pid, Ref, Ready) ->
    gen_server:cast(Pid, {send_ready, Ref, Ready}).
