...
sort_addrs(TID, Addrs) ->
    ...
    %% additionally only allow an address to appear once in case several transports claim it
    lists:foldl(fun(Elements, Acc) -> Acc ++ [E || E <- Elements, not lists:member(E, Acc)] end, [], SortedAddrLists).
