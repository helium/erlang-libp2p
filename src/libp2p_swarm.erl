...
-type ref() :: libp2p_swarm_sup:ref().

-spec start(string() | non_neg_integer()) -> ref().
start(Addr) ->
...
-spec listen(ref(), string() | non_neg_integer()) -> ok | {error, term()}.
...
% Connect
%
-spec add_connection_handler(ref(), string(), {libp2p_transport:connection_handler(), libp2p_transport:connection_handler()}) -> ok.
...
% Stream
%
-spec dial(ref(), string(), string()) -> {ok, libp2p_connection:connection()} | {error, term()}.
dial(Sup, Addr, Path) ->
    % e.g. dial(SID, "/ip4/127.0.0.1/tcp/5555", "echo")
    Server = libp2p_swarm_sup:server(Sup),
    libp2p_swarm_server:dial(Server, Addr, Path).

-spec add_stream_handler(ref(), string(), libp2p_session:stream_handler()) -> ok.
add_stream_handler(Sup, Key, HandlerDef) ->
    Server = libp2p_swarm_sup:server(Sup),
    libp2p_swarm_server:add_stream_handler(Server, Key, HandlerDef).

-spec stream_handlers(ref()) -> [{string(), libp2p_session:stream_handler()}].
stream_handlers(Sup) ->
    Server = libp2p_swarm_sup:server(Sup),
